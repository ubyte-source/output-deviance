	...
	class Deviance {
		...
		constructor() {
			...
			this.getTable().getThead().addRow(
				...
				window.Table.Tr.Th.content(Period.odd()),
				window.Table.Tr.Th.content(Period.even()),
				window.Table.Tr.Th.content(Count.odd()),
				window.Table.Tr.Th.content(Count.even()),
				window.Table.Tr.Th.content(Dictionary.delta()),
				window.Table.Tr.Th.content(Calculate.negative()),
				window.Table.Tr.Th.content(Calculate.positive())
			);
		}
		...
		calculate() {
			...
			for (let item = 0; item < result.length; item++) {
				let label = item > 0 ? Average.positive() : Average.negative(),
					colspan = 6 + item,
					parameters = [
